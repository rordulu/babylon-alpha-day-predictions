...
larger_ratio_features = ["200D-Ratio", "MOM3", "MACD_SIG", "Next-5-Day-Close-To-Todays-Close-Ratio", "MACD-Prev-MACD"]
unknown_range_features = ["MACD", "SIG_LINE", "Volatility-20", "Volatility-5", "Volume", "MACD_SIG"]
...
def make_new_features_for(data, underlying_target=underlying_target_def, verbose=False):
    ...
    data["Volatility-20"] = data["Close"].pct_change().rolling(window=20).std()
    data["Volatility-5"] = data["Close"].pct_change().rolling(window=5).std()
...
#    data['Date'] = pd.to_datetime(data['Date'], format=None, errors='coerce', utc=True).dt.tz_localize(None)
#
...
#    data["Threshold"] = data["Threshold"].apply(lambda x: min(x, 0.025))
    ...
    data[dm_target] = (data[underlying_target] >= fixed_threshold).astype(float) #& (data['Close'] > data['Open'])).astype(float)
...
def construct_values_for_model(sequence_size, ticker_symbols = limited_stocks, underlying_target=underlying_target_def, filenames = None, use_for_last_day_prediction=False, verbose=False, refresh=False, data_interval="1y", find_sectors=False, is_binary_prediction=True):
    ...
    for i, ticker in enumerate(ticker_symbols):
        ...
        x_for_ticker = make_new_features_for(x_for_ticker, underlying_target=underlying_target, verbose=verbose)
