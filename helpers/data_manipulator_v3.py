...
lstm_features_v2 = [
 "High-Low-Ratio", "High-Open-Ratio", "Close-Open-Ratio",
  ...
  ,"Volatility-10", "Volatility-20", "Volatility-50", "Volatility-100","Volatility-200"
 , "MACD_SIG"
    ,"MFI3", "RSI", "RSI3", "POCR"
    ,"fastk", "fastd", "SlowK1"
    , "willr3", "willr2", "willr1"
...
small_oscillate_features = ["MACD_HIST_NORM", "MACD-Increase-Prev-MACD", "eps_to_prev_eps", "Next-Day-Close-To-Next-Day-Open-Ratio", "Next-Day-High-To-Next-Day-Open-Ratio"]
...
extra_informational_features = ["Next-Day-Open", "Next-Day-Close-To-Next-Day-Open-Ratio", "Next-Day-High-To-Next-Day-Open-Ratio", "Next-Day-Close", "Next-Day-Low-To-Next-Day-Open-Ratio", "today_is_earning_day_evening", "today_is_earning_day_morning"]
...
def make_new_features_for(data, underlying_target=underlying_target_def, verbose=False, earnings=None, use_fixed_mean=True, binary_target_ma_period=20, use_adj_close = False):
...
#    data["Threshold"] = data["Threshold"].apply(lambda x: min(x, 0.025))
    ...
    if use_fixed_mean:
        ...
        data[dm_target] = (data[underlying_target] * 1.001 >= fixed_threshold).astype(float) #& (data['Close'] > data['Open'])).astype(float)
...
def construct_values_for_model(sequence_size, ticker_symbols = limited_stocks, underlying_target=underlying_target_def, filenames = None, use_for_last_day_prediction=False, verbose=False, refresh=False, data_interval="1y", find_sectors=False, is_binary_prediction=True, incl_earnings=False, use_fixed_mean=True, binary_target_ma_period=20):
    ...
    for i, ticker in enumerate(ticker_symbols):
        ...
        x_for_ticker = make_new_features_for(x_for_ticker, underlying_target=underlying_target, verbose=verbose, earnings=earnings_for_ticker, use_fixed_mean=use_fixed_mean, binary_target_ma_period=binary_target_ma_period)
